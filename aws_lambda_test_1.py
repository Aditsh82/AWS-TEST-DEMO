...
def get_data():
    ...
    s3_bucket = "random-users-data-527620741250"
    ...
    ############### GOOD TEST ###################################
    # data_file = open('s3_files' + '.txt', 'w+')
    # data_file.write(str(",".join(i for i in s3_files_list)))
    # data_file.close()
    
    # s3.upload_file('s3_files.txt', s3_bucket , 'data_file')
    ##############################################################

    # pp = "".join(i for i in s3_files_list)
    # objects.put(Body= bytes(pp,"utf-8"))
    ##############################################################
